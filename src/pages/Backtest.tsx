...
> = {
  ...
  trend: {
    ...
    activeClass: "bg-foreground text-background",
    badgeClass: "text-neon-green bg-neon-green/10",
...
function SymbolTable({ bySymbol }: { bySymbol: Record<string, any> }) {
  ...
  const th = (k: SortKey, label: string) => (
    <th
      className="px-4 py-3 text-left font-sans text-[11px] tracking-wider text-muted-foreground cursor-pointer hover:text-primary transition-colors whitespace-nowrap"
      ...
      <table className="w-full text-xs">
        ...
        <tbody>
          {rows.map(r => (
            <tr
              key={r.symbol}
              className="border-b border-border/10 hover:bg-muted/60 transition-colors"
            >
              ...
              <td className="px-4 py-3 font-mono">
                <span
                  className={
                    r.winRate >= 0.5 ? "text-neon-green" : "text-red-400"
                  }
                >
              ...
              <td className="px-4 py-3 font-mono">
                <span
                  className={
                    r.avgReturn >= 0 ? "text-neon-cyan" : "text-red-400"
                  }
                >
                  {r.avgReturn >= 0 ? "+" : ""}
              ...
              <td className="px-4 py-3 font-mono">
                <span
                  className={
                    r.expectancy >= 0 ? "text-neon-cyan" : "text-red-400"
                  }
                >
                  {r.expectancy >= 0 ? "+" : ""}
...
function TradeRow({ trade }: { trade: any }) {
  ...
  return (
    <tr className="border-b border-border/10 hover:bg-muted/60 transition-colors">
      ...
      <td className="px-2 py-1.5 font-mono text-[10px]">
        <span
          className={cn(
            exitColors[trade.exitReason as string] ?? "text-muted-foreground"
          )}
        >
      ...
      <td className="px-2 py-1.5 font-mono text-xs">
        <span
          className={
            (trade.returnPct as number) >= 0 ? "text-neon-cyan" : "text-red-400"
          }
        >
          {(trade.returnPct as number) >= 0 ? "+" : ""}
          {(trade.returnPct as number).toFixed(2)}%
        </span>
      </td>
...
export default function Backtest() {
  ...
  // ── tRPC ────────────────────────────────────────────
  const { data: pastRuns, isLoading: runsLoading } =
    trpc.backtest.list.useQuery(undefined, { refetchInterval: 30000 });
  ...
  const handleRun = () => {
    ...
    runMutation.mutate({
      ...
      outcomeWindowCandles:
        config.outcomeWindowCandles || defaultWindow[config.tf] || 42,
            ...
            <div className="space-y-4">
              ...
              <div>
                  ...
                  {(Object.keys(STRATEGY_META) as StrategyOption[]).map(s => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "text-left font-mono text-[10px] px-3 py-2 rounded-lg transition-all bg-muted/50 hover:bg-muted",
                          active ? meta.activeClass : "text-muted-foreground"
                        )}
                      >
                ...
                <p className="font-mono text-[10px] text-muted-foreground/60 mt-1.5">
                  각 전략별 시그널 발생 시점을 lookahead-free 로 재생하여 정밀한
                  승률 측정
                </p>
              </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {Object.entries(SYMBOL_PRESETS).map(([label, val]) => (
                    <button
                      key={label}
                      onClick={() =>
                        setConfig(c => ({ ...c, symbolsText: val }))
                      }
                      className={cn(
                        "font-mono text-[10px] px-3 py-1.5 rounded-full transition-all",
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e =>
                    setConfig(c => ({ ...c, symbolsText: e.target.value }))
                  }
                  rows={2}
                  placeholder="BTCUSDT,ETHUSDT,SOLUSDT"
                  className="w-full font-mono text-xs bg-muted rounded-md px-3 py-2 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none"
                />
                <p className="font-mono text-[10px] text-muted-foreground/60 mt-1">
                  {config.symbolsText.split(",").filter(s => s.trim()).length}개
                  심볼 선택됨
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full font-mono text-xs bg-muted rounded-md px-3 py-2 text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20"
                  >
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setConfig(c => ({
                        ...c,
                        outcomeWindowCandles: parseInt(e.target.value) || 42,
                      }))
                    }
                    min={5}
                    max={200}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setConfig(c => ({
                        ...c,
                        cooldownCandles: parseInt(e.target.value) || 5,
                      }))
                    }
                    min={1}
                    max={50}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setConfig(c => ({ ...c, runName: e.target.value }))
                    }
                    placeholder="my_backtest"
                    className="w-full font-mono text-xs bg-muted rounded-md px-3 py-2 text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary/20"
                  />
                </div>
                <div className="flex items-end pb-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={e =>
                        setConfig(c => ({ ...c, saveToDb: e.target.checked }))
                      }
              ...
              <div className="flex items-center gap-3 pt-1">
                <Button
                  onClick={handleRun}
                  disabled={isRunning}
                  className="h-9 px-6 font-display font-bold"
                >
                  {isRunning ? (
                    <>
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              {[
                ...
                <div key={item.step} className="p-3 rounded-lg bg-card">
                  <div
                    className={cn(
                        ...
                        <div className="space-y-1 mt-2">
                          {[
                            ...
                            const pct = result.trades.length
                              ? ((count / result.trades.length) * 100).toFixed(
                                  1
                                )
                              : "0";
                            return (
                    ...
                    <thead className="sticky top-0 bg-background/90 backdrop-blur">
                      <tr className="border-b border-border/20">
                        {[
                          ...
                          <th
                            key={h}
                            className="px-4 py-3 text-left font-sans text-[11px] tracking-wider text-muted-foreground"
                          >
                            {h}
                          </th>
                        ))}
              ...
              >
                <div className="p-3 rounded-lg bg-neon-yellow/5">
                  <p className="font-mono text-xs text-neon-yellow mb-2">
                    베이스라인 승률:{" "}
                    <strong>{(overall.winRate * 100).toFixed(1)}%</strong> (
                    {overall.totalTrades}개 시그널)
                  </p>
                  <p className="font-mono text-[10px] text-muted-foreground leading-relaxed">
                    이 수치가 Calibration의{" "}
                    <code className="text-primary">baselineWinRate</code>가
                    됩니다. Trend Composite (EMA200, ADX, MACD 등 7개 지표 합성)
                    추가 시 regime별 승률 비교 →
                    <code className="text-foreground"> strong_uptrend</code>에서
                    예상 승률:{" "}
              ...
              <div className="space-y-2">
                {pastRuns.map(run => {
                  ...
                  return (
                    <div
                      key={run.id}
                      className="flex items-center justify-between p-3 rounded-lg bg-card hover:bg-muted/40 transition-all"
                    >
